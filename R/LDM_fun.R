...
#'
#'#-------------------------------------------------------------------------
#'# Testing mediation of the microbiome in the effect of smoking on Outcome
#'#-------------------------------------------------------------------------
#'res.ldm.mediation <- ldm(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus + Outcome, 
#'                       data=throat.meta, seed=67817, 
#'                       test.mediation=TRUE) # parameter for requesting mediation analysis (LDM-med)
#'                       
#'#-------------------------------------------------------------------------
#'# The new omnibus test (LDM-omni3)
#'#-------------------------------------------------------------------------
#'res.ldm.omni3 <- ldm(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus+PackYears, 
#'                     data=throat.meta, seed=67817, n.cores=4,  # seed=123 for the published results
#'                     test.omni3=TRUE) # parameter for requesting LDM-omni3
#'(tab.global <- cbind(res.ldm.omni3$p.global.freq, res.ldm.omni3$p.global.tran, res.ldm.omni3$p.global.pa, 
#'                     res.ldm.omni3$p.global.harmonic, res.ldm.omni3$p.global.fisher,
#'                     res.ldm.omni3$p.global.omni, res.ldm.omni3$p.global.omni3))
#'(tab.otu <- c(length(res.ldm.omni3$detected.otu.freq[[1]]), 
#'              length(res.ldm.omni3$detected.otu.tran[[1]]), 
#'              length(res.ldm.omni3$detected.otu.pa[[1]]),
#'              length(res.ldm.omni3$detected.otu.omni[[1]]), 
#'              length(res.ldm.omni3$detected.otu.omni3[[1]])))
#'              
#'#-------------------------------------------------------------------------
#'# Mediation analysis (LDM-med)
#'#-------------------------------------------------------------------------
#'res.ldm.med <- ldm(formula = throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus + Outcome,
#'                   data=throat.meta, seed=67817, n.cores=4,
#'                   test.mediation=TRUE)  # parameter for requesting 
#'res.ldm.med$med.p.global.omni 
#'res.ldm.med$med.detected.otu.omni 
#'
#'### Calling both LDM-med and LDM-omni3
#'res.ldm.med.omni3 <- ldm(formula = throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus + Outcome,
#'                         data=throat.meta, seed=67817, n.cores=4,
#'                         test.mediation=TRUE, # parameter for requesting mediation analysis 
#'                         test.omni3=TRUE)     # parameter for requesting LDM-omni3
#'(tab.global <- c(res.ldm.med.omni3$med.p.global.freq, 
#'                 res.ldm.med.omni3$med.p.global.tran, 
#'                 res.ldm.med.omni3$med.p.global.pa, 
#'                 res.ldm.med.omni3$med.p.global.harmonic, 
#'                 res.ldm.med.omni3$med.p.global.fisher,
#'                 res.ldm.med.omni3$med.p.global.omni, 
#'                 res.ldm.med.omni3$med.p.global.omni3))
#'(tab.otu <- c(length(res.ldm.med.omni3$med.detected.otu.freq), 
#'              length(res.ldm.med.omni3$med.detected.otu.tran), 
#'              length(res.ldm.med.omni3$med.detected.otu.pa),
#'              length(res.ldm.med.omni3$med.detected.otu.omni), 
#'              length(res.ldm.med.omni3$med.detected.otu.omni3)))
#'                       
#'#-------------------------------------------------------------------------
#'# Testing associations with a survival outcome
#'#-------------------------------------------------------------------------
#'
#'library(survival)
#'cox.fit = coxph(Surv(Time, Status) ~ Sex+AntibioticUse, data=throat.meta)
#'resid.Martingale = residuals(cox.fit, type='martingale')
#'resid.deviance = residuals(cox.fit, type='deviance')
#'res.ldm.surv <- ldm(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ resid.Martingale, 
#'                    data=throat.meta, seed=67817, n.cores=4,
#'                    test.omni3=TRUE,                # requesting LDM-omni3
#'                    other.surv.resid=resid.deviance) # requesting analysis of the other residual and the combination test
#'res.ldm.surv$p.global.omni3      
#'res.ldm.surv$p.global.omni3.OR   
#'res.ldm.surv$p.global.omni3.com  
#'res.ldm.surv$detected.otu.omni3      
#'res.ldm.surv$detected.otu.omni3.OR   
#'res.ldm.surv$detected.otu.omni3.com  
#'
#'#-------------------------------------------------------------------------
#'# Testing mediation of the microbiome in the effect of smoking on a survival outcome 
#'#-------------------------------------------------------------------------
#'res.ldm.surv.med <- ldm(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus + resid.Martingale, 
#'                        data=throat.meta, seed=67817, n.cores=4,
#'                        other.surv.resid=resid.deviance, # the other residual and the combination test
#'                        test.mediation=TRUE,             # mediation analysis
#'                        test.omni3=TRUE)                # LDM-omni3
#'res.ldm.surv.med$n.perm.completed 
#'res.ldm.surv.med$med.p.global.omni3      
#'res.ldm.surv.med$med.p.global.omni3.OR   
#'res.ldm.surv.med$med.p.global.omni3.com  
#'res.ldm.surv.med$med.detected.otu.omni3  
#'res.ldm.surv.med$med.detected.otu.omni3.OR   
#'vres.ldm.surv.med$med.detected.otu.omni3.com  
#'res.ldm.surv.med$p.global.omni3.com
...
ldm = function( formula, other.surv.resid=NULL, data=.GlobalEnv, tree=NULL, dist.method="bray", dist=NULL, 
    ...
    if (!all.rarefy | test.omni3) {
        ...
        ldm.obs.freq = ldm.stat(x=x.design, low=low, up=up, resid=resid.freq, ss.tot=ss.tot.freq, adjust.for.confounders=adjust.for.confounders)
        ...
        if (!is.null(OR)) {
            ldm.obs.freq.OR = ldm.stat(x=x.design.OR, low=low, up=up, resid=resid.freq.OR, ss.tot=ss.tot.freq.OR, adjust.for.confounders=adjust.for.confounders)
    ...
    if (ifelse(is.null(n.perm.max), 1, (n.perm.max>0))) {
        ...
        parallel.perm <- function(i) {
            ...
            if (!all.rarefy | test.omni3) {
                ldm.perm.freq = ldm.stat(x=x.design[perm[i,], ], low=low, up=up, resid=resid.freq[,otu.smallp,,,drop=FALSE], ss.tot=ss.tot.freq[,otu.smallp,,drop=FALSE], adjust.for.confounders=adjust.for.confounders)
                ...
                if (!is.null(OR)) {
                    ldm.perm.freq.OR = ldm.stat(x=x.design.OR[perm[i,], ], low=low, up=up, resid=resid.freq.OR[,otu.smallp,,,drop=FALSE], ss.tot=ss.tot.freq.OR[,otu.smallp,,drop=FALSE], adjust.for.confounders=adjust.for.confounders)
        ...
        while (n.perm.completed < n.perm.max) {
            ...
            if (n.otu.smallp > 20 & n.cores > 1) { 
                ...
                if (Sys.info()[['sysname']] == 'Windows') {
                    parallel.stat = bplapply(1:n.perm.block, parallel.perm, BPPARAM = MulticoreParam(workers=n.cores))
                } else {
                    parallel.stat = mclapply(1:n.perm.block, parallel.perm, mc.cores = n.cores)
                }
                ...
                for (i in 1:n.perm.block) {
                    ...
                    if (!all.rarefy | test.omni3) {
                        ldm.perm.freq = ldm.stat(x=x.design[perm[i,], ], low=low, up=up, resid=resid.freq[,otu.smallp,,,drop=FALSE], ss.tot=ss.tot.freq[,otu.smallp,,drop=FALSE], adjust.for.confounders=adjust.for.confounders)
                        if (!is.null(OR)) ldm.perm.freq.OR = ldm.stat(x=x.design.OR[perm[i,], ], low=low, up=up, resid=resid.freq.OR[,otu.smallp,,,drop=FALSE], ss.tot=ss.tot.freq.OR[,otu.smallp,,drop=FALSE], adjust.for.confounders=adjust.for.confounders)
                        if (!freq.scale.only) {
                ...
                if (meet.rej.stop | (n.perm.completed >= n.global.perm.max)) {
                    ...
                    if (!all.rarefy | test.omni3) {
                        ...
                        if (!is.null(OR)) {
                            ...
                            p.global.freq.null <- n.perm.completed + 0.5 - apply(global.freq.perm[,,1:n.perm.completed,drop=FALSE], c(1,2), rank)
                            p.global.freq.null <- aperm(p.global.freq.null, c(2,3,1))
                            p.global.freq.null.OR <- n.perm.completed + 0.5 - apply(global.freq.perm.OR[,,1:n.perm.completed,drop=FALSE], c(1,2), rank)
                            p.global.freq.null.OR <- aperm(p.global.freq.null.OR, c(2,3,1))
                        ...
                        if (!freq.scale.only) {
                            ...
                            if (!is.null(OR)) {
                                ...
                                p.global.tran.null <- n.perm.completed + 0.5 - apply(global.tran.perm[,,1:n.perm.completed,drop=FALSE], c(1,2), rank)
                                p.global.tran.null <- aperm(p.global.tran.null, c(2,3,1))
                                p.global.tran.null.OR <- n.perm.completed + 0.5 - apply(global.tran.perm.OR[,,1:n.perm.completed,drop=FALSE], c(1,2), rank)
                                p.global.tran.null.OR <- aperm(p.global.tran.null.OR, c(2,3,1))
                    ...
                    if (test.omni3 | (!all.rarefy & !freq.scale.only)) {
                        ...
                        if (!is.null(OR)) {
                            ...
                            n.global.omni.OR <- rowSums( (pmin.global.omni.null.OR < c(pmin.global.omni.OR) - tol.eq) + 0.5 * (abs(pmin.global.omni.null.OR - c(pmin.global.omni.OR)) < tol.eq), dims=2) # apply( (pnullmin.global.omni < c(pmin.global.omni) - tol.eq), c(1,2), sum) + 0.5 * apply( (abs(pnullmin.global.omni - c(pmin.global.omni)) < tol.eq), c(1,2), sum)
                        ...
                        if (test.omni3) {
                            ...
                            otu.freq.sd <- apply(otu.freq.perm[,,1:n.perm.completed,drop=FALSE], c(1,2), sd)
                            ...
                            otu.tran.sd <- apply(otu.tran.perm[,,1:n.perm.completed,drop=FALSE], c(1,2), sd)
                            ...
                            otu.pa.sd <- apply(otu.pa.perm[,,1:n.perm.completed,drop=FALSE], c(1,2), sd)
                            ...
                            if (!is.null(OR)) {
                                otu.freq.sd.OR <- apply(otu.freq.perm.OR[,,1:n.perm.completed,drop=FALSE], c(1,2), sd)
                                ...
                                otu.tran.sd.OR <- apply(otu.tran.perm.OR[,,1:n.perm.completed,drop=FALSE], c(1,2), sd)
                                ...
                                otu.pa.sd.OR <- apply(otu.pa.perm.OR[,,1:n.perm.completed,drop=FALSE], c(1,2), sd)
                    ...
                    if (test.mediation) {
                        ...
                        if (!all.rarefy | test.omni3) {
                            ...
                            med.T.otu.freq = apply(p.otu.freq, 2, max)
                            ...
                            med.global.freq.perm = apply(1/med.T.otu.freq.null, 2, sum) # length: B
                            ...
                            if (!is.null(OR)) {
                                ...
                                med.T.otu.freq.OR = apply(p.otu.freq.OR, 2, max)
                                ...
                                med.global.freq.perm.OR = apply(1/med.T.otu.freq.null.OR, 2, sum) # length: B
                            ...
                            if (!freq.scale.only) {
                                ...
                                med.T.otu.tran = apply(p.otu.tran, 2, max) # I_j
                                ...
                                med.global.tran.perm = apply(1/med.T.otu.tran.null, 2, sum) # I^(b)
                                ...
                                if (!is.null(OR)) {
                                    ...
                                    med.T.otu.tran.OR = apply(p.otu.tran.OR, 2, max) # I_j
                                    ...
                                    med.global.tran.perm.OR = apply(1/med.T.otu.tran.null.OR, 2, sum) # I^(b)
                        ...
                        if (all.rarefy | test.omni3) {
                            ...
                            med.T.otu.pa = apply(p.otu.pa, 2, max)
                            ...
                            med.global.pa.perm = apply(1/med.T.otu.pa.null, 2, sum) # length: B
                            ...
                            if (!is.null(OR)) {
                                ...
                                med.T.otu.pa.OR = apply(p.otu.pa.OR, 2, max)
                                ...
                                med.global.pa.perm.OR = apply(1/med.T.otu.pa.null.OR, 2, sum) # length: B
                        ...
                        if (test.omni3 | (!all.rarefy & !freq.scale.only)) {
                            ...
                            if (test.omni3) {
                                ...
                                med.global.harmonic.perm = apply(1/med.pmin.otu.omni3.null, 2, sum) # B
                                print(length(med.global.harmonic.perm))
                                ...
                                med.global.fisher.perm = apply(-log(med.pmin.otu.omni3.null), 2, sum)
                                ...
                                if (!is.null(OR)) {
                                    ...
                                    med.global.harmonic.perm.OR = apply(1/med.pmin.otu.omni3.null.OR, 2, sum) # B
                                    ...
                                    med.global.fisher.perm.OR = apply(-log(med.pmin.otu.omni3.null.OR), 2, sum)
    ...
    if (!is.null(q.otu.freq)) dimnames(q.otu.freq) = p.otu.names
    ...
    if (!is.null(OR)) {
        ...
        if (!is.null(q.otu.freq.OR)) dimnames(q.otu.freq.OR) = p.otu.names
        ...
        if (!is.null(q.otu.freq.com)) dimnames(q.otu.freq.com) = p.otu.names
...
#'dist.J.sq <- res.jaccard$jac.mean.sq.o2
#'res.perm.D2A2 <- permanovaFL(dist.J.sq | (Sex+AntibioticUse) ~ SmokingStatus+PackYears, 
...
#'
#'#----------------------------------------------------------------
#'# Omnibus test of multiple distance matrix (two ways of specifying distances)
#'#----------------------------------------------------------------
#'res.perm.omni <- permanovaFL(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus+PackYears, 
#'                        data=throat.meta, seed=82955,
#'                        dist.method=c("bray", "jaccard"))
#'                        
#'dist.J.sq = jaccard.mean( throat.otu.tab5 )$jac.mean.sq.o2
#'dist.BC =  as.matrix(vegdist(x=throat.otu.tab5 / rowSums(throat.otu.tab5), method="bray"))
#'dist.BC.sq = dist.BC^2  
#'res.perm.omni.alt <- permanovaFL(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus+PackYears, 
#'                       data=throat.meta, seed=82955, 
#'                       dist.list=list(dist.J.sq, dist.BC.sq), square.dist=FALSE)   
#'                       
#'#----------------------------------------------------------------
#'# Testing mediation of the microbiome between an exposure and an outcome
#'#----------------------------------------------------------------       
#'res.perm.med.omni <- permanovaFL(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus+Outcome, 
#'                       data=throat.meta, seed=67817, dist.list=list(dist.J.sq, dist.BC.sq), square.dist=FALSE,
#'                       test.mediation=TRUE)    
#'                       
#'#----------------------------------------------------------------
#'# Testing associations with a survival outcome
...
#'resid.deviance = residuals(cox.fit, type='deviance')
#'
#'res.perm.omni.surv <- permanovaFL(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ resid.Martingale, 
#'                                      other.surv.resid=resid.deviance, # requesting analysis of the other residual and the combination test
#'                                      seed=82955, data=throat.meta, 
#'                                      dist.method=c("bray", "jaccard"))  # omnibus test of multiple distances
#'                       
#'#----------------------------------------------------------------
#'# Testing mediation of the microbiome between an exposure and a survival outcome
#'#----------------------------------------------------------------  
#'   
#'res.perm.med.omni.surv <- permanovaFL(formula=throat.otu.tab5 | (Sex+AntibioticUse) ~ SmokingStatus+resid.Martingale, 
#'                                      seed=82955, data=throat.meta, 
#'                                      other.surv.resid=resid.deviance,  # Deviance residual, combination test
#'                                      dist.method=c("bray", "jaccard"),  # omnibus test of multiple distances
#'                                      test.mediation=TRUE,               # mediation
#'                                      n.rarefy=2)                        # rarefaction   
#'res.perm.med.omni.surv$p.permanova                # (n.dist x n.var1 x n.rarefy)
#'res.perm.med.omni.surv$p.permanova.OR
#'res.perm.med.omni.surv$p.permanova.com
#'
#'res.perm.med.omni.surv$p.permanova.omni           # (n.var1 x n.rarefy)
#'res.perm.med.omni.surv$p.permanova.omni.OR
#'res.perm.med.omni.surv$p.permanova.omni.com
#'
#'res.perm.med.omni.surv$med.p.permanova            # (n.dist x n.rarefy)
#'res.perm.med.omni.surv$med.p.permanova.OR
#'res.perm.med.omni.surv$med.p.permanova.com
#'
#'res.perm.med.omni.surv$med.p.permanova.omni       # (n.rarefy)
#'res.perm.med.omni.surv$med.p.permanova.omni.OR
#'res.perm.med.omni.surv$med.p.permanova.omni.com                        
    ...
    if (n.perm.max > 0) {
        ...
        for (i.block in 1:nblock) {
            ...
            if (n.cores > 1) { # parallel computing
                ...
                if (Sys.info()[['sysname']] == 'Windows') {
                    parallel.stat = bplapply(1:n.perm.block, parallel.perm, BPPARAM = MulticoreParam(workers=n.cores), x.design, perm, low, up, resid.dist, ndf, adjust.for.confounders)
                } else {
                    parallel.stat = mclapply(1:n.perm.block, parallel.perm, mc.cores = n.cores, x.design, perm, low, up, resid.dist, ndf, adjust.for.confounders)
                }
                ...
                if (!is.null(OR)) {
                    if (Sys.info()[['sysname']] == 'Windows') {
                        parallel.stat.OR = bplapply(1:n.perm.block, parallel.perm, BPPARAM = MulticoreParam(workers=n.cores), x.design.OR, perm, low, up, resid.dist.OR, ndf, adjust.for.confounders)
                    } else {
                        parallel.stat.OR = mclapply(1:n.perm.block, parallel.perm, mc.cores = n.cores, x.design.OR, perm, low, up, resid.dist.OR, ndf, adjust.for.confounders)
                    }
